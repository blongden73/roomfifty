...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('compile-handlebars', 'Compile Handlebars templates ', function() {
    ...
    //helpers
    handlebars.registerHelper('saleEach', function(context, options) {
      var ret = "";

      for(var i=0; i < context.length; i++) {
        if(context[i].tags != "" && context[i].tags.includes("sale")){
          ret = ret + options.fn(context[i]);
          // console.log('sale', context[i].title);
          // console.log(ret);
        }
      }
    ...
    handlebars.registerHelper('IWDEach', function(context, options) {
      var ret = "";

      for(var i=0; i < context.length; i++) {
        if(context[i].tags != "" && context[i].tags.includes("IWD")){
          ret = ret + options.fn(context[i]);
          // console.log('sale', context[i].title);
          // console.log(ret);
        }
    ...
    handlebars.registerHelper('IWDEachProduct', function(context, options) {
      var ret = "";
    ...
    handlebars.registerHelper("sale", function(title) {
        if(title != '') {
          var g = title.toLowerCase().split(", ");
          var sale = g.find(function(string){
              if(string.startsWith('sale')){
                  return string.startsWith('sale');
              }
           });
           // console.log(tote);
           if(sale != undefined){
    ...
    handlebars.registerHelper("IWD", function(title) {
        if(title != '') {
          var g = title.toLowerCase().split(", ");
          var iwd = g.find(function(string){
              if(string.startsWith('IWD')){
                  return string.startsWith('IWD');
              }
           });
           // console.log(tote);
           if(iwd != undefined){
    ...
    handlebars.registerHelper("gift", function(title) {
        if(title != '') {
          var g = title.toLowerCase().split(", ");
          var gift = g.find(function(string){
              if(string.startsWith('gift')){
                  return string.startsWith('gift');
              }
           });
           var tote = g.find(function(string){
               if(string.startsWith('tote')){
                   return string.startsWith('tote');
               }
           });
           // console.log(tote);
           if(gift != undefined){
    ...
    handlebars.registerHelper("findURL", function(title, dev) {
        if(title != '') {
         var t = title.split(", ");
         console.log(t, 'title split');
         if(title.indexOf('http') > 1 || title.indexOf('roomfifty') > 1) {
